...
int Thermal (int os, int argc, char *argv[], char path[1024], char outputpath[1024], int warnings, int recover, int NR, double dr_grid,
	...
	int diss_type = 2;                   // Switch for fluid tidal dissipation type: 0 = kinetic drag only (tilalpd/r), 1 = potential energy dissipation of barotropic waves (tilalpp), 2 = both
	...
	double *r_old = (double*) malloc((NR+1)*sizeof(double)); // Old grid zone radii memorized during the porosity compaction calculation (cm)
	if (r_old == NULL) printf("Thermal: Not enough memory to create r_old[NR+1]\n");

	double *pore_old = (double*) malloc(NR*sizeof(double));  // Old porosity memorized during the porosity compaction calculation (unitless)
	if (pore_old == NULL) printf("Thermal: Not enough memory to create pore_old[NR+1]\n");
...
//						((*Mrock)[ir] + (*Mh2os)[ir] + (*Mh2ol)[ir])/(4.0/3.0*PI_greek*(pow((*r)[ir+1],3.0)-pow((*r)[ir],3.0)))/(1.0-(*pore)[ir]), ((*Mrock)[ir] + (*Mh2os)[ir] + (*Mh2ol)[ir])/((*Vrock)[ir] + (*Vh2os)[ir] + (*Vh2ol)[ir]),
//						((*Mrock)[ir] + (*Mh2os)[ir] + (*Mh2ol)[ir])/dVol[ir], (*pore)[ir]);
//	}

	// Memorize *r, not necessarily uniform in spacing if starting in recovery mode from an output not generated by IcyDwarf
	for (ir=0;ir<NR+1;ir++) {
		r_old[ir] = (*r)[ir];
//		if (ir < NR) printf("%d \t %g \t %g \t %g\n", ir, (*r)[ir+1], (*pore)[ir], (*Vrock)[ir]/dVol[ir]);
	}
	for (ir=0;ir<NR;ir++) pore_old[ir] = (*pore)[ir];
...
//		printf("%d \t %g \t %g \t %g\n", ir, (*r)[ir+1], (*pore)[ir], creep_rate);
	}

	// Update radii
	for (ir=0; ir<NR;ir++) {
//		(*r)[ir+1] = (*r)[ir] + gridstep[ir]*pow(1.0-(*pore)[ir],-1.0/3.0);
		(*r)[ir+1] = pow(pow((*r)[ir],3.0) + (pow(r_old[ir+1],3.0)-pow(r_old[ir],3.0))*(1.0-pore_old[ir])/(1.0-(*pore)[ir]) , 1.0/3.0);
//		printf("%d \t %g \t %g\n", ir, (*r)[ir+1], (*pore)[ir]);
	}
...
//	ir_ocean = *ircore+1;
	...
	if (itime > 0 && Mprim && (eorb > 0.0 || obl > 0.0) && !moonspawn && ir_ocean > *ircore) {
		...
		ocean_mid = (int) floor((double) (ir_ocean + (*ircore)))/2.0; // If there is an ocean, there is essentially no porosity from the center to the base of the ice, so we can assume that the midpoint in radii is also the midpoint in grid zone number
		r_ocean = (*r)[ocean_mid];            // Radius of mid-depth in the ocean (cm)
		if (r_ocean <= 0.0) r_ocean = DBL_EPSILON;
...
//			(*tilT) = pow(10.0, Tpow);
		...
		if (obl > DBL_EPSILON) {
			TROPF(*cesq, *tilT, diss_type, -tilom, 2, 1, 1.5, &(W_fluidtide[3]), &knFsF, &kLovenF); // G21W term (retrograde), Legendre polynomial P21 = 1.5
			TROPF(*cesq, *tilT, diss_type,  tilom, 2, 1, 1.5, &(W_fluidtide[4]), &knFsF, &kLovenF); // G21E term (prograde), Legendre polynomial P21 = 1.5
		}

		sumW_fluidtide = + (-1.5    *eorb*0.5) * (-1.5    *eorb*0.5) * W_fluidtide[0] // P20 = -0.5 instead?
...
//			Eh2osnew[jr] = Eh2osnew[jr] - q*(*Eh2os)[ir];
			Volcell[jr] = 0.0;
//			printf("1st ice pass, moving one cell up at ir = %d\t jr = %d\t q = %g\n", ir, jr, q);
			jr++;
...
//			Eh2osnew[jr] = Eh2osnew[jr] - q*(*Eh2os)[ir];
			Volcell[jr] = 0.0;
//			printf("2nd ice pass, moving one cell up at ir = %d\t jr = %d\t q = %g\n", ir, jr, q);
			jr++;
			Vh2osnew[jr] = q*(*Vh2os)[ir];
			Mh2osnew[jr] = q*(*Mh2os)[ir];
//			Eh2osnew[jr] = q*(*Eh2os)[ir];
